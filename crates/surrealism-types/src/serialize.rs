...
use crate::arg::{Arg, SerializableArg};
use crate::controller::MemoryController;
use crate::transfer::{Ptr, Transfer};
use surrealdb::expr;
use surrealdb::protocol::{ToFlatbuffers, FromFlatbuffers};
use anyhow::Result;
...
impl<T: Arg> Serializable for SerializableArg<T> {
    fn serialize(self) -> Result<Serialized> {
        self.0.to_value().serialize()
    }

    fn deserialize(serialized: Serialized) -> Result<Self> {
        Ok(SerializableArg(T::from_value(expr::Value::deserialize(serialized)?)?))
    }
}
...
impl Serializable for expr::Kind {
    fn serialize(self) -> Result<Serialized> {
    ...
    fn deserialize(serialized: Serialized) -> Result<Self> {
        ...
        Ok(expr::Kind::from_fb(value)?)
    }
}

impl Serializable for expr::Value {
    fn serialize(self) -> Result<Serialized> {
        let mut builder = flatbuffers::FlatBufferBuilder::with_capacity(1024);
        let root = self.to_fb(&mut builder)?;
        builder.finish(root, None);
