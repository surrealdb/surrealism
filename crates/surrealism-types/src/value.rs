...
impl Transferrable for sql::Value {
    ...
    fn from_transferrable(value: Value, controller: &mut dyn MemoryController) -> Result<Self> {
        match value {
            ...
            Value::SR_VALUE_ARRAY(x) => {
                Ok(Self::Array(sql::Array::from_transferrable(x, controller)?))
            }
            Value::SR_VALUE_OBJECT(x) => Ok(Self::Object(sql::Object::from_transferrable(
                x, controller,
            )?)),
            Value::SR_VALUE_BYTES(x) => {
                Ok(Self::Bytes(sql::Bytes::from_transferrable(x, controller)?))
            }
            Value::SR_VALUE_THING(x) => {
                Ok(Self::Thing(sql::Thing::from_transferrable(x, controller)?))
            }
        }
    }
}
