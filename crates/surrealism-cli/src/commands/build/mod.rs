...
fn load_config(path: &PathBuf) -> Result<SurrealismConfig> {
    ...
    SurrealismConfig::parse(
        &fs::read_to_string(&surrealism_toml)
            .with_context(|| "Failed to read surrealism.toml")?
    )
}
...
fn optimize_wasm(source_wasm: &PathBuf) -> Result<Vec<u8>> {
    ...
    // Read and strip WASM
    let wasm_bytes = fs::read(source_wasm)
        .with_context(|| "Failed to read WASM file")?;
...
fn apply_wasm_opt(wasm_bytes: &[u8]) -> Result<Vec<u8>> {
    ...
    fs::read(&temp_wasm_output)
        .with_context(|| "Failed to read optimized WASM file")
}

fn get_source_wasm(path: &PathBuf) -> Result<PathBuf> {
    let metadata = metadata(path)
        .with_context(|| "Failed to retrieve cargo metadata")?;
    ...
    let current_package = packages
        ...
        .ok_or_else(|| anyhow::anyhow!("Could not find current package in metadata for path: {}", path.display()))?;
