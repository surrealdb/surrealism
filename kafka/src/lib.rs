...
//! local development).
//!
//! Register with e.g. `DEFINE MODULE kafka AS f"bucket:/kafka.surli";` and
//! call `kafka::produce("10.0.0.5:9092", "events", "user-42", "{...}")`.
//! `broker` must be a literal IP address: guest code cannot resolve
//! hostnames (WASI's ip-name-lookup is disabled to prevent DNS-tunnelling
//! exfiltration — see the `webhook` module's README for the same
//! constraint and its workaround for hostname-based targets).
...
fn produce_impl(broker: &str, topic: &str, key: Option<&str>, value: &str) -> Result<i64, String> {
	...
	let record_batch =
		encode_record_batch(key.map(str::as_bytes), Some(value.as_bytes()), current_timestamp_millis());
...
#[surrealism]
fn produce(broker: String, topic: String, key: String, value: String) -> Result<i64, String> {
	let key = if key.is_empty() { None } else { Some(key.as_str()) };
	produce_impl(&broker, &topic, key, &value)
}
